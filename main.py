...
class KNN:
    ...
    def predict(self, X):
        #find the likely labels for the data in X and return it in an array
...
if __name__ == "__main__":
    ...
    def accuracy(y_true, y_pred):
        ...
        return accuracy
    def getFolds(i, datasize, data):
        start = i * datasize
        end = start + datasize
        train = data.drop(start, end)
    ...
    X = data[features].split()
    y = data['class'].split()

    #für folds
    folds = 5
    datasize = int(X.shape[0] / folds)
    liste = list()
    for i in folds:
