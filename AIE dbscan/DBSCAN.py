...
class CustomDBSCAN():
    ...
    # Find all neighbour points at given distance
    def neighbour_points(self, data, pointId, distance):
        points = []
        for i in range(len(data)):
            # Euclidian distance
            if np.linalg.norm([a_i - b_i for a_i, b_i in zip(data[i], data[pointId])]) <= distance:
                points.append(i)
        return points

    # Fit the data into the DBSCAN model
    def fit(self, data, distance, MinPt):
        ...
        # Find the neighbours of each individual point in given distance
        for i in range(len(data)):
            point_count.append(self.neighbour_points(data, i, distance))
        ...
        # differentiate between border point and outlier
        for i in border:
...
def main():
    ...
    point_labels, clusters = custom_DBSCAN.fit(X, 0.3, 4)
